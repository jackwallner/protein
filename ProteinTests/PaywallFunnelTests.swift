...
final class PaywallFunnelTests: XCTestCase {
    ...
    func testCountsPitchesPerSurfaceAndInTotal() {
        ConversionDiagnostics.recordPitchView(impressionID: "protein_plus_tab")
        ConversionDiagnostics.recordPitchView(impressionID: "protein_plus_tab")
        ConversionDiagnostics.recordPitchView(impressionID: "protein_paywall")

        let attributes = ConversionDiagnostics.subscriberAttributes
        XCTAssertEqual(attributes["pitch_views_total"], "3")
        XCTAssertEqual(attributes["pitch_views_plus_tab"], "2")
        XCTAssertEqual(attributes["pitch_last"], "paywall")
    }
    ...
    func testConversionFreezesTheSurfaceAndCountAtTheMomentOfSale() {
        ...
        let attributes = ConversionDiagnostics.subscriberAttributes
        XCTAssertEqual(attributes["converted_surface"], "paywall")
    ...
    func testAttributeKeysStayInsideRevenueCatsLimit() {
        ...
        ConversionDiagnostics.recordPitchView(impressionID: "protein_\(long)")
